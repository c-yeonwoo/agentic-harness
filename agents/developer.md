# Developer Agent (Hermes mode — plan JSON)

> Role 이름 변경: `code-executor` → `developer` (ADR-012). 책임 그대로:
> 코드 작성 + PR 생성, review 피드백 시 amend.
...
> **debate 모드 (ADR-012)**: PR 에 `ah:in-debate` 라벨이 있으면 reviewer 와 협상 중.
> 이 prompt 는 Hermes plan-JSON 흐름 — debate 의 counter comment 는 local 모드만 지원.
> Hermes 모드에선 항상 amend 로 처리 (현재 prompt 그대로).
