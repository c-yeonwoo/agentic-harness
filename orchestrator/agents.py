...
# ── PO mode B (merged PR → SoT 갱신 PR) — ADR-017 ────────────────────────────


async def run_po_mode_b(
    repo: str,
    repo_cwd: Path,
    pr_numbers: list[int],
    mode: str = "urgent",                  # urgent (즉시 단일) | batch (주간 통합)
    model: Optional[str] = None,
) -> dict:
    """merged PR 들의 변경을 분석하고 SoT 파일 갱신 PR 자동 생성.

    Returns: {ok, summary, pr_url?, files_changed, error?, cost_usd}
    """
    if not (repo_cwd / ".git").exists():
        return {"ok": False, "error": f"repo cwd 가 git repo 아님: {repo_cwd}"}
    ...
    # 1) target PR 들 fetch
    target_prs: list[dict] = []
    for n in pr_numbers:
        try:
            pr = await gh.get_pr(repo, n)
            diff = await gh.pr_diff(repo, n, max_bytes=15_000)
            files = await gh.pr_files(repo, n)
            target_prs.append({
                "number": pr.number,
                "title": pr.title,
                "body": pr.body,
            ...
            log.warning("po.mode_b.fetch_failed", number=n, error=str(exc))

    if not target_prs:
        return {"ok": False, "error": "분석할 PR 가져오기 실패"}
    ...
    try:
        ...
        log.warning("po.mode_b.sot_discover_failed", error=str(exc))

    # 3) worktree 분기 (신규 branch — SoT 갱신용)
    from orchestrator.git_apply import (
        prepare_worktree, stage_commit_push_all, cleanup_worktree, detect_worktree_changes,
    )
    import time
    ts = int(time.time())
    branch_name = f"chore/sot-update-{mode}-{ts}"

    try:
        wt = await prepare_worktree(repo_cwd=repo_cwd, branch_name=branch_name)
    except Exception as exc:
        log.exception("po.mode_b.worktree_failed", error=str(exc))
        return {"ok": False, "error": f"worktree 준비 실패: {exc}"}

    try:
        # 4) claude -p (PO mode B) — worktree 안에서 SoT 파일 Edit/Write
        from orchestrator.runners.local_claude import run_po_mode_b_local
        res = await run_po_mode_b_local(
            repo_cwd=wt.path,
            repo=repo,
        ...
        if changes["files_changed"] == 0:
            log.info("po.mode_b.no_changes", repo=repo, mode=mode,
                     summary=res.summary)
            # 처리는 했지만 변경 없음 — sot-urgent/batch 라벨은 호출자가 정리
            return {
        ...
        # 6) commit + push
        prs_tag = ", ".join(f"#{p['number']}" for p in target_prs)
        commit_msg = res.pr_title or f"docs(sot): {mode} update — {prs_tag}"
        push_info = await stage_commit_push_all(
            repo_cwd=repo_cwd, wt=wt, commit_message=commit_msg,
        )

        # 7) PR 생성
        pr_body = (res.pr_body or "").rstrip()
        pr_body += f"\n\n---\n_🤖 Generated by agentic-harness PO mode B ({mode})_"
        pr_body += f"\n_분석한 PR: {prs_tag}_"
        pr_body += f"\n_cost ${res.cost_usd:.4f} · {res.input_tokens} in / {res.output_tokens} out · model={res.model}_"

        pr = await gh.create_pr(
            repo,
            title=res.pr_title or f"docs(sot): {mode} update — {prs_tag}",
            body=pr_body,
            head=push_info["branch"],
            base=push_info["base"],
            labels=["ah:needs-review"],
        )
        ...
        return {
            ...
            "analyzed_prs": [p["number"] for p in target_prs],
            "cost_usd": res.cost_usd,
            "model": res.model,
            "todos": res.todos,
        }
...
async def run_code_reviewer(
    ...
    try:
        try:
            ...
            # SoT 영향도 라벨 부착 (ADR-017) — verdict 와 직교
            # high → ah:sot-urgent (merge 시 PO mode B 즉시 트리거)
            # medium → ah:sot-batch (주간 batch 큐)
            # low → 라벨 없음 (SoT 영향 없음)
            sot_impact = (review.get("sot_impact") or "low").lower()
            sot_label = {"high": "ah:sot-urgent", "medium": "ah:sot-batch"}.get(sot_impact)
            if sot_label:
                # 이전 sot 라벨 정리 (medium 이었는데 다시 보니 high 로 변경 등)
                for old in ("ah:sot-urgent", "ah:sot-batch"):
                    if old != sot_label:
                        try:
                            await gh.remove_label(repo, "pr", pr.number, old)
                        except Exception:
                            pass
                try:
                    await gh.add_label(repo, "pr", pr.number, sot_label)
                    log.info("reviewer.sot_label",
                             pr=pr.number, impact=sot_impact, label=sot_label)
                except Exception as exc:
                    log.warning("reviewer.sot_label_failed",
                                pr=pr.number, error=str(exc))
            else:
                # low 면 기존 sot 라벨 있어도 제거 (재평가에서 영향 없다 판정)
                for old in ("ah:sot-urgent", "ah:sot-batch"):
                    try:
                        await gh.remove_label(repo, "pr", pr.number, old)
                    except Exception:
